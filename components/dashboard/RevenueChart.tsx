export function RevenueChart() {
  const points = "20,250 95,210 170,215 245,205 295,185 330,110 385,70 455,58 540,62";
    ...
    <section className="h-[424px] rounded-2xl bg-white p-6 shadow-[0_8px_24px_rgba(91,58,153,0.10)]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {["7 Days", "30 Days", "90 Days"].map((label, index) => (
            <button
              key={label}
              className={`h-8 rounded-[8px] px-4 text-[14px] font-medium ${
                index === 0 ? "bg-brand text-white" : "bg-surface-soft text-muted"
              }`}
            >
              {label}
            </button>
